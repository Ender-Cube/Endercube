...
public class MapVoteInventory {
    ...
    public MapVoteInventory() {
        ...
        mainView.fill(inventory, slot -> ItemStack.of(Material.BLACK_STAINED_GLASS_PANE).withDisplayName(Component.empty()));
        maps.fill(inventory, slot -> ItemStack.of(Material.MAP)); // just for mockup
        
        // Init action bar
        actions.set(inventory, 0,
                ItemStack.of(Material.SEA_PICKLE).withDisplayName(Component.text("Pick all").color(NamedTextColor.GREEN))
        );
        actions.set(inventory, 4,
                ItemStack.of(Material.BARRIER).withDisplayName(Component.text("Close").color(NamedTextColor.RED))
        );
        actions.set(inventory, 8,
                Heads.QUESTION_MARK.getItemStack().withDisplayName(Component.text("Help!").color(NamedTextColor.GREEN))
        );
